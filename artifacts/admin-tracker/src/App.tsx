  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <header className="border-b border-border bg-card/60 backdrop-blur-md sticky top-0 z-10">
        <div className="max-w-6xl mx-auto px-4 h-14 flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <span className="font-bold text-foreground text-sm">AI Academy Tracker</span>
          </div>
          ...
          <nav className="flex gap-1">
            {NAV.map(n => (
              <button
                key={n.id}
                onClick={() => setTab(n.id)}
                className={`h-8 px-3 text-xs rounded-lg flex items-center gap-1.5 transition-colors font-medium ${tab === n.id ? "bg-primary/10 text-primary" : "text-muted-foreground hover:text-foreground"}`}
              >
                <span>{n.icon}</span>
      ...
      <footer className="border-t border-border py-4 text-center text-xs text-muted-foreground">
        AI Academy Admin Tracker · Web &amp; Mobile analytics
      </footer>
    </div>
  );
}
