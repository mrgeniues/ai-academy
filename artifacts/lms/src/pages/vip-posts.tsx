...
import {
  ...
  Smile, ImageIcon, Paperclip, X, Loader2, Crown, Lock, Link2, ExternalLink,
  FileText, Download
} from "lucide-react";
    ...
    <>
      ...
      {(fileUrl || docFile) && (
        <div className="relative mt-2 flex items-center gap-3 p-3 bg-muted rounded-xl border border-border">
          ...
          {onRemoveDoc && (
            <button onClick={onRemoveDoc} className="flex-shrink-0 text-muted-foreground hover:text-destructive transition-colors p-1">
              <X className="w-3.5 h-3.5" />
            </button>
          )}
        </div>
...
function CommentSection({ postId, token }: { postId: number; token: string | null }) {
  ...
  const renderCommentBody = (comment: Comment, isReply: boolean) => (
    <div className="flex gap-2">
      ...
      <div className={`flex-1 rounded-lg px-3 py-2 min-w-0 ${isReply ? "bg-muted/50 border border-amber-200/30" : "bg-muted"}`}>
  ...
  return (
    <div className="mt-3 pt-3 border-t border-amber-200/30 space-y-3">
      {loading ? (
          ...
          {topLevelComments.map(comment => (
            <div key={comment.id}>
              ...
              {(repliesMap.get(comment.id) ?? []).length > 0 && (
                <div className="ml-9 mt-2 space-y-2 border-l-2 border-amber-200/40 pl-3">
                ...
                <div className="ml-9 mt-2">
                  <div className="flex gap-2">
                    <Avatar className="w-6 h-6 flex-shrink-0 mt-1">
                      <AvatarFallback className="text-[10px] bg-amber-100 text-amber-700 dark:bg-amber-900/50 dark:text-amber-300">
      ...
      <div className="flex gap-2">
        <Avatar className="w-7 h-7 flex-shrink-0">
          ...
          <AvatarFallback className="text-xs bg-amber-100 text-amber-700 dark:bg-amber-900/50 dark:text-amber-300 font-semibold">
            {user?.name?.slice(0, 2).toUpperCase()}
          </AvatarFallback>
        </Avatar>
        <div className="flex-1 space-y-1.5">
          <div className="flex items-center gap-1 bg-muted rounded-lg px-3 py-1.5">
            ...
            <div className="flex items-center gap-0.5 flex-shrink-0">
              ...
              <button type="button" onClick={() => setShowCommentLink(s => !s)}
                className={`transition-colors p-1 rounded ${showCommentLink ? "text-amber-600" : "text-muted-foreground hover:text-foreground"}`}
                title="Add link">
              ...
              <button type="button" onClick={() => docInputRef.current?.click()}
                className="text-muted-foreground hover:text-foreground transition-colors p-1 rounded" title="Attach document">
                <Paperclip className="w-4 h-4" />
              </button>
              <button onClick={handleSubmit} disabled={!canSubmitMain}
                className="text-amber-600 hover:text-amber-500 disabled:opacity-30 transition-colors p-1 rounded">
          ...
          {showCommentLink && (
            <div className="flex items-center gap-1.5 px-3 py-1.5 bg-muted rounded-lg border border-amber-300/40">
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API}/posts`, {
        method: "POST",
        headers: { ...authHeaders(token), "Content-Type": "application/json" },
        body: JSON.stringify({ content: fullContent, imageUrl, fileUrl, fileType, isVip: true }),
      });
      if (!res.ok) throw new Error();
            ...
            <CardContent className="pt-4 pb-4 px-4">
              <div className="flex gap-3">
                ...
                <div className="flex-1 space-y-2">
                  ...
                  {/* Link input */}
                  {showLinkInput && (
                    <div className="flex items-center gap-2 px-3 py-2 bg-muted rounded-xl border border-amber-300/40">
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-1">
                      ...
                      <button
                        ...
                        className={`transition-colors p-1 rounded ${showLinkInput ? "text-amber-500" : "text-muted-foreground hover:text-foreground"}`}
                        title="Add link"
                      >
                        <Link2 className="w-4 h-4" />
                      </button>
